...
bool bInImage(int u, int v, int w, int h)
{
  if(u >= 0 && u < w && v >= 0 && v < h) {
      return true;
  } else {
...
int main(int argc, char **argv) {
    ...
    cout << "1 image1 keypoints: " << keypoints.size() << endl;
    ...
    cv::imshow("features1", image_show);
    ...
    cout << "image2 keypoints: " << keypoints2.size() << endl;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    cout<<"matches size: "<<matches.size()<<" Match time cost= "<<time_used.count()<<" seconds."<<endl;
    // END YOUR CODE HERE
    for (auto &m: matches) {
